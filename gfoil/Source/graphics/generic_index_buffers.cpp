...
void gfoil::generic_index_buffers::generate_index_buffers() {
	...
	if (config::data.count("low_memory_mode") == 0)
		config::data["low_memory_mode"] = "false";
	if (config::data["low_memory_mode"] == "false")
	...
	if (low_memory_mode) {
		...
		generic_index_buffers::quad.buffer_data(max_indices * sizeof(uint16_t), &indices[0], buffer::draw_types::STATIC);
	} else {
		uint32_t max_indices = 40000 * 6;
		...
		generic_index_buffers::quad.generate(buffer::types::INDEX);
		generic_index_buffers::quad.buffer_data(max_indices * sizeof(uint32_t), &indices[0], buffer::draw_types::STATIC);
	}
	...
	if (low_memory_mode) {
		...
		generic_index_buffers::cube.buffer_data(max_indices * sizeof(uint16_t), &indices[0], buffer::draw_types::STATIC);
	} else {
		uint32_t max_indices = 10000 * 36;
		std::vector<uint32_t> indices;
		indices.resize(max_indices);
		...
		generic_index_buffers::cube.generate(buffer::types::INDEX);
		generic_index_buffers::cube.buffer_data(max_indices * sizeof(uint32_t), &indices[0], buffer::draw_types::STATIC);
	}
